...
static void find_as(u8* argv0) { //负责查找 as 并用 afl-as 替换

  u8 *afl_path = getenv("AFL_PATH"); //获取环境变量
  u8 *slash, *tmp;

  if (afl_path) {

    tmp = alloc_printf("%s/as", afl_path); //在同目录下的alloc-inl.h中，找到afl_path/as赋值给tmp
  ...
  if (slash) {
    ...
    *slash = '/';
    ...
    tmp = alloc_printf("%s/afl-as", dir); //是同目录下的alloc-inl.h中DFL_ck_strdup()，创建一个字符串副本的缓冲区
...
static void edit_params(u32 argc, char** argv) {
  ...
  //后面通过name选择使用哪个编译器:LLVM/Clang、gcc、g++、gcj
